...
class TransactionHistoryPagingSource(private val networkRepo: NetworkRepo) : PagingSource<Int, TransactionHistoryResponse.Detail.Data.Transaction>() {
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, TransactionHistoryResponse.Detail.Data.Transaction> {
        val currentPage = params.key ?: 1
        return try {
            val response = networkRepo.fetchTransactionHistory(page = currentPage, limit = params.loadSize)
